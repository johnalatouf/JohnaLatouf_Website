	...
	var winWidth = $(window).width();    //window width
	 ...
	 var NAVPADPERCENT = 0.03; //this determines the padding of the nav links
		...
		//push the foreground up for vertical screen 
	 	if ( winWidth<400 ) {
	 		$(".foreground").css("background-position", "center 250px");
	 	} else {
	...
	//when you are already on a link, slide to that stuff
   	if(window.location.hash){
   		...
   		if ( $(window).width() < 600 && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
   			if ( $(window).width() < 400 ) {
   				$(".foreground").stop().animate({ left: - ( targetJump*0.63 ) }, 3000);
   	...
   	$("#menu-main-menu a").bind("click",function(event) {
   		...
   		if ( $(window).width() < 600 && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
   			if ( $(window).width() < 400 ) {
   				$(".foreground").stop().animate({ left: - ( targetJump*0.64 ) }, 3000);
				$("#masthead").stop().animate({ left: + ( targetJump/20 ) }, 1200);
				$("#colophon").stop().animate({ left: + ( targetJump/20 ) }, 1200);
   			} else {
   				$(".foreground").stop().animate({ left: - ( targetJump*0.79 ) }, 3000);
