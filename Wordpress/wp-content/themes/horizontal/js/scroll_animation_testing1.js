...
$(window).on("load resize",function(e){
	...
	//correct the body width for huge screens
	if (winWidth >= 1600) {
		$('body').css('width', winWidth*5);
		$('.panel').css('padding-left', 40);
		$('.panel').css('padding-right', 90);
	} else {
		$('body').css('width', '8000px');
		$('.panel').css('padding-left', 20);
		$('.panel').css('padding-left', 15);
	}
	 ...
	 var NAVPADPERCENT = 0.03; //this determines the padding of the nav links
		...
		//push the foreground up for vertical screen 
	 	if ( winWidth<400 ) {
	 		$(".foreground").css("background-position", "center 280px");
	 	} else {
	 		$(".foreground").css("background-position", "bottom");
	 	}
	 	...
	 	$(".background").css("background-position", "top");
   	 	...
   	 	panMarg = 20;
   	 	$(".panel").css("margin-left", panMarg);
   	 	$(".scrollable").css("font-size", "70%");
   	 	$(".scrollable").css("padding", "10px 50px 10px 20px");
	...
	//when you are already on a link, slide to that stuff
   	if(window.location.hash){
   		...
   		if ( $(window).width() < 600 && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
   			if ( $(window).width() < 400 ) {
   				$(".foreground").stop().animate({ left: - ( targetJump*0.66 ) }, 3000);
